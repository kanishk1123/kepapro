...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      document.querySelector(".navlink").removeEventListener("mousemove", updateCursorPosition);
    };
  }, []); 
...
< nav className={`flex text-2xl justify-between h-18 py-3 text-white overflow-hidden bg-black relative ${search ? 'max-md:h-[200px]' : 'max-md:h-18'}`}>
  {/* Navigation content */}
     ...
     <button
      ...
      <form action="" method='post'  className={`p2 flex  justify-center  duration-600  gap-2 items-center`} onSubmit={(e)=>submithandler(e)}>
        <input className='px-2 w-0 duration-500 -mt-2 py-1 rounded-xl placeholder:text-zinc-400 bg-transparent border-2 border-zinc-400' style={widht} placeholder='searach here' type="text"  />
        <button
